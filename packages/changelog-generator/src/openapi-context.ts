...
function buildLineMap(specYaml: string): LineMap {
  ...
  for (const [pathStr, pathObj] of Object.entries(spec.paths)) {
    ...
    for (const [method, methodObj] of Object.entries(
      ...
      const pathLineRegex = new RegExp(
        `^\\s*${pathStr.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}:\\s*$`,
      );
      ...
      for (let i = methodLineNum; i < lines.length; i++) {
        ...
        if (
          indent <=
            lines[methodLineNum - 1].length -
              lines[methodLineNum - 1].trimStart().length &&
          i > methodLineNum
        ) {
          break;
        }
        ...
        if (trimmed.startsWith('responses:')) {
          ...
          responsesStart > 0 &&
          responsesEnd === -1 &&
          (trimmed.startsWith('requestBody:') ||
            trimmed.startsWith('parameters:'))
        ) {
          responsesEnd = i;
        }
      ...
      if (requestBodyEnd === -1 && requestBodyStart > 0)
        requestBodyEnd = lines.length;
      if (responsesEnd === -1 && responsesStart > 0)
        responsesEnd = lines.length;
      if (parametersStart > 0 && parametersEnd === -1)
        parametersEnd = lines.length;
      ...
      for (let lineNum = methodLineNum; lineNum < endLine; lineNum++) {
        let location: 'request' | 'response' | 'parameter' | 'unknown' =
          'unknown';
        if (
          requestBodyStart > 0 &&
          lineNum >= requestBodyStart &&
          lineNum < requestBodyEnd
        ) {
          location = 'request';
        } else if (
          responsesStart > 0 &&
          lineNum >= responsesStart &&
          lineNum < responsesEnd
        ) {
          location = 'response';
        } else if (
          parametersStart > 0 &&
          lineNum >= parametersStart &&
          lineNum < parametersEnd
        ) {
          location = 'parameter';
        }
...
export function enrichChangesWithContext(
  ...
  for (const change of changes) {
    ...
    if (property === 'path') {
      ...
      const lineNumber =
        change.context?.originalLine || change.context?.newLine;
