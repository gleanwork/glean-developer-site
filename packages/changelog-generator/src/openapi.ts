...
function extractBulletsFromDiff(diff: any): Array<string> {
  ...
  for (const key of arrays) {
    ...
    for (const item of arr) {
      ...
      if (type === 'PathItem' && path)
        bullets.push(`${capitalize(key)} path: ${path}`);
      else if (type === 'Operation' && method && path)
        bullets.push(`${capitalize(key)} operation: ${method} ${path}`);
      else if (type === 'Schema' && v.name)
        bullets.push(`${capitalize(key)} schema: ${v.name}`);
      else if (type === 'Parameter' && v.name && v.in)
        bullets.push(`${capitalize(key)} parameter: ${v.name} (${v.in})`);
      else if (type === 'Response' && v.code)
        bullets.push(`${capitalize(key)} response: ${v.code}`);
    }
  }
  ...
  if (pathsNode && typeof pathsNode === 'object') {
    for (const pathName of Object.keys(pathsNode)) {
      ...
      const opsContainer =
        pEntry.operations || pEntry.changed || pEntry.added || pEntry;
      for (const m of methodKeys) {
        const op = opsContainer?.[m] || opsContainer?.[m.toUpperCase()];
        if (!op) continue;
        const isChanged =
          typeof op === 'boolean' ? op : Object.keys(op).length > 0;
        if (isChanged)
          bullets.push(`Changed operation: ${m.toUpperCase()} ${pathName}`);
      }
    }
  }
  ...
  if (Array.isArray(schemasNode.added)) {
    for (const s of schemasNode.added)
      bullets.push(`Added schema: ${String(s)}`);
  }
  if (schemasNode.changed && typeof schemasNode.changed === 'object') {
    for (const sName of Object.keys(schemasNode.changed)) {
      ...
      for (const prop of propsAdded)
        bullets.push(`Schema ${sName}: added property ${String(prop)}`);
    }
  }

  // Fallback summary when structure differs but we still know there are changes
  if (bullets.length === 0 && hasMeaningfulChanges(diff)) {
    const pathAdded = countArrayLen(
      diff.added,
      (i: any) => i?.type === 'PathItem',
    );
    const opAdded = countArrayLen(
      diff.added,
      (i: any) => i?.type === 'Operation',
    );
    const schemaAdded = countArrayLen(
      diff.added,
      (i: any) => i?.type === 'Schema',
    );
    const opChanged = countArrayLen(
      diff.changed,
      (i: any) => i?.type === 'Operation',
    );
    const schemaChanged = countArrayLen(
      diff.changed,
      (i: any) => i?.type === 'Schema',
    );
...
export async function ingestOpenApiCommits(opts: {
  ...
  const useDiff =
    opts.cfg.diffEnabled !== false && engine === 'openapi-changes' && !isVitest;
      ...
      try {
        ...
        for (const f of Array.from(rec.files)) {
          ...
          try {
            ...
            diffToolFailures++;
            dbgOpenApi(
              'diff: runner error for %s file %s %o',
              sha.slice(0, 7),
              f,
              e,
            );
          ...
          if (diff != null) {
            if (hasMeaningfulChanges(diff)) {
              ...
              commitDiffs
                .get(sha)!
                .push({ baseYaml: baseContent, headYaml: headContent, diff });
            }
          }
        }
        if (!meaningful) {
  ...
  for (const { sha, message, date, files } of commitMap.values()) {
    ...
    dayBuckets
      .get(date)!
      .push({ sha, message: msg, files: Array.from(files).sort(), diffs });
  }
  ...
  return {
    files,
    report: { days: days.length, commits: totalCommits, diffToolFailures },
  };
}
