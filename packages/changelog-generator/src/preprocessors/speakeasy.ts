...
export function isSpeakeasyFormat(text: string): boolean {
  // Must have either "Speakeasy" mention or the backtick-method structural pattern
  const hasSpeakeasyMention = /speakeasy/i.test(text);
  const hasMethodPattern = /^\*\s+`[^`]+\(\)`/m.test(text);
  return hasSpeakeasyMention && hasMethodPattern;
}
