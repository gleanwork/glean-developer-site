...
function repairUnclosedTrailingBrackets(field: string): string {
  const openBrackets = (field.match(/\[/g) || []).length;
  const closeBrackets = (field.match(/\]/g) || []).length;
  const hasTrailingUnclosedBracket =
    field.lastIndexOf('[') > field.lastIndexOf(']');

  if (hasTrailingUnclosedBracket && openBrackets > closeBrackets) {
    return `${field}${']'.repeat(openBrackets - closeBrackets)}`;
  }
  ...
  for (const line of lines) {
    ...
    if (singleLineMatch) {
      changes.push({
        ...
        field: repairUnclosedTrailingBrackets(singleLineMatch[2]),
    ...
    if (indentedMatch && currentMethod) {
      changes.push({
        ...
        field: repairUnclosedTrailingBrackets(indentedMatch[1]),
