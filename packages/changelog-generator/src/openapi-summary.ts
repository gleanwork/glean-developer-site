...
type GroupedChanges = Map<EndpointKey, {
	endpoint: EndpointContext;
	changes: Array<ContextualChange>;
}>;

export function analyzeOpenApiChangesWithContext(contextualChanges: Array<ContextualChange>): AnalyzedChanges {
	...
	for (const [key, { endpoint, changes }] of sortedEntries) {
		if (endpoint.method === 'ALL') {
			for (const change of changes) {
				const action = change.changeType === 'added' ? 'Added' : change.changeType === 'removed' ? 'Removed' : 'Modified';
			...
			for (const change of changes) {
				const action = change.changeType === 'added' ? 'Added' : change.changeType === 'removed' ? 'Removed' : 'Modified';
				const locationStr = endpoint.location !== 'unknown' ? ` (${endpoint.location})` : '';
				const breakingMark = change.breaking ? ' ⚠️' : '';
				
				if (change.itemType === 'field') {
					details.push(`  - ${action} field: \`${change.itemName}\`${locationStr}${breakingMark}`);
				} else if (change.itemType === 'parameter') {
					details.push(`  - ${action} parameter: \`${change.itemName}\`${breakingMark}`);
				}
			}
		}
	}
	...
	for (const change of changes) {
		...
		if (property === 'path') {
			...
			if (changeText.includes('added')) {
				...
				if (change.original) details.push(`Removed endpoint: ${change.original}`);
			} else {
			...
			if (changeText.includes('added')) {
				...
				if (change.original) details.push(`Removed parameter: ${change.original}`);
			} else {
...
function generateSummary(
	...
	if (stats.endpointsAdded > 0) {
		parts.push(`${stats.endpointsAdded} endpoint${stats.endpointsAdded > 1 ? 's' : ''} added`);
	}
	if (stats.endpointsRemoved > 0) {
		parts.push(`${stats.endpointsRemoved} endpoint${stats.endpointsRemoved > 1 ? 's' : ''} removed`);
	}
	if (stats.endpointsModified > 0) {
		parts.push(`${stats.endpointsModified} endpoint${stats.endpointsModified > 1 ? 's' : ''} modified`);
	}

	if (stats.fieldsAdded > 0) {
		parts.push(`${stats.fieldsAdded} field${stats.fieldsAdded > 1 ? 's' : ''} added`);
	}
	if (stats.fieldsRemoved > 0) {
		parts.push(`${stats.fieldsRemoved} field${stats.fieldsRemoved > 1 ? 's' : ''} removed`);
	}
	if (stats.fieldsModified > 0) {
		parts.push(`${stats.fieldsModified} field${stats.fieldsModified > 1 ? 's' : ''} modified`);
	}

	if (stats.parametersAdded > 0) {
		parts.push(`${stats.parametersAdded} parameter${stats.parametersAdded > 1 ? 's' : ''} added`);
	}
	if (stats.parametersRemoved > 0) {
		parts.push(`${stats.parametersRemoved} parameter${stats.parametersRemoved > 1 ? 's' : ''} removed`);
	}
	if (stats.parametersModified > 0) {
		parts.push(`${stats.parametersModified} parameter${stats.parametersModified > 1 ? 's' : ''} modified`);
	}
...
export function formatChangeCategories(categories: Set<ChangeCategory>): string {
