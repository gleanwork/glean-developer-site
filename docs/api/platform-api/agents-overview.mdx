---
title: Agents API Overview
---

import ApiOverview from '@site/src/components/ApiOverview';

# Agents API

<ApiOverview
  title="Agents API"
  description="The Agents API lets you discover Glean agents, inspect their input and output schemas, and execute agent runs. Use it to embed Glean's agentic capabilities directly into your own applications and workflows."
  apiFamily="agents"
  apiType="platform-api"
  useCases={[
    {
      title: 'Agent Discovery',
      description:
        'Search for agents available to the authenticated user and retrieve their configuration to decide which agent to invoke.',
      icon: 'Search',
    },
    {
      title: 'Schema Inspection',
      description:
        'Fetch an agent\u2019s input and output schemas so you can build type-safe integrations and validate payloads before invoking a run.',
      icon: 'FileText',
    },
    {
      title: 'Agent Execution',
      description:
        'Create agent runs to execute Glean agents programmatically and incorporate their results into downstream automation.',
      icon: 'Play',
    },
  ]}
/>

For per-user integrations, authenticate with OAuth; the [Platform API authentication guide](/api/platform-api/authentication) explains the Glean OAuth Authorization Server and external-IdP options. Use Glean-issued tokens only for approved admin-managed or global-permission exceptions.

For error responses, see [Platform API Errors](/errors) for the shared ProblemDetail code reference and remediation guidance.
