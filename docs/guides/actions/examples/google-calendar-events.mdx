...
<Card title="OAuth Setup" icon="Key">
  Critical OAuth implementation details: - Always use `access_type=offline` for
  refresh token support - Request minimal scopes (readonly for calendar access)
  - Handle token refresh scenarios gracefully - Implement proper token storage
  and security
</Card>
