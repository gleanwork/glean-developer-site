...
To create or update an agent from a specification directory, package the directory as a zip and send it to the [Import an agent](/api/client-api/agents/import-agent) endpoint.
...
| `trigger` | Yes, for the top-level agent | How the agent is invoked. Always declare it explicitly. Subagents must not declare it. |
...
`trigger` controls how the top-level agent is invoked. Always declare it
explicitly, including for chat agents. Subagents are never triggered directly
and must not declare a `trigger`.
