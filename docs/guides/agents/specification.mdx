...
There is no separate `MANUAL` trigger type.
...
and `CONTAINS` apply to those fields. At most 20 trigger entries are allowed.

Do not declare `trigger.schedule` for content-triggered agents. Runtime fields
such as `content`, `eventTime`, and `system.event` are managed internally and
must not be declared as input fields.
...
- `frequency` is a required five-field Unix cron expression. Cron descriptors
  and six-field expressions with seconds are not supported.
...
Each skill directory must contain a `SKILL.md` file. The display name on the UI
is derived from the folder name, converted from kebab-case to title case.
