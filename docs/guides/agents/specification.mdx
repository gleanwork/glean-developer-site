...
The Glean agent specification is a human-readable file system format for defining a Glean agent — its instructions, tools, knowledge sources, skills, dependencies, and subagents — as a directory of files. Use it to author and version-control Glean agents outside the UI.

To create or update an agent from a specification directory, zip the directory so the archive contains one top-level agent folder, and send it to the [Import an agent](/api/client-api/agents/import-agent) endpoint.
...
| --- | --- | --- |
| `id` | Yes | Stable unique agent identifier: a 32-character lowercase hex string, such as a `uuid.uuid4().hex` value. Do not change it after creation. |
| `name` | Yes | Human-readable agent name shown to users. At most 100 characters. |
| `description` | No | Short description of what the agent does. At most 300 characters. |
| `instructionFile` | No | Path to the instructions file. Defaults to `instructions.md`. The file is limited to 32,000 characters. |
| `model` | No | Which agentic model the agent runs on and how hard it reasons. |
| `trigger` | No | How the top-level agent is invoked. Defaults to `CHAT_MESSAGE` when omitted; declaring it explicitly is recommended. Subagents do not use it. |
...
dependencies:
  ...
  - type: agent
    id: 0f1e2d3c4b5a69788776655443322110
...
| Key | Description |
| --- | --- |
| `name` | Agentic model set ID. Tenant-specific; omit it to use the tenant default. |
| `mode` | Reasoning effort per turn. `ADVANCED` (default) or `FAST`. |
| `autoUpgrade` | Boolean. Omit it by default. See the rules below. |
...
- Without `name`, omit `autoUpgrade`. The tenant controls model upgrades.
- With `name`, omitting `autoUpgrade` pins the agent to that model.
- Set `autoUpgrade: true` to opt a named model into upgrades.
- `autoUpgrade: false` pins explicitly and requires `name`; a spec with `false` and no `name` is rejected.
...
`trigger` controls how the top-level agent is invoked. It defaults to
`CHAT_MESSAGE` when omitted; declaring it explicitly, even for chat agents,
keeps the intent clear. Subagents are never triggered directly, so a
`trigger` in a subagent's `spec.yaml` has no effect; leave it out.

Supported `type` values:

| `type` | Description |
| --- | --- |
| `CHAT_MESSAGE` | The user converses with the agent. |
...
`triggers` accepts at most 20 entries. Each content trigger requires
`datasource` and `templateId`. `docType` is required when the subscribed event
does not specify a document type. `timeOffset` and `conditions` are optional. A `conditions` entry requires
...
- `frequency` is a required five-field Unix cron expression. Cron descriptors
  such as `@daily` and six-field expressions with a seconds column are rejected.
...
| `type` | Yes | enum | — | One of `TEXT`, `SELECT`, `DATE`, or `DOCUMENT`. |
...
| `DOCUMENT` | A file the agent reads on each run. The value is a Glean file ID, not the file contents. |
...
- `toolProviderId`: identifier of the tool provider, such as an MCP server. Use `native` for Glean's built-in tools.
...
Each skill directory must contain a `SKILL.md` file with frontmatter `name`
(at most 128 characters) and `description` (at most 1,000 characters). The
display name is the title-cased folder name; for example,
`lead-qualification` becomes "Lead Qualification".
