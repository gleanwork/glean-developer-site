...
| `suggested`       | Filter by suggestion. Use the value `my history` for documents in the current user's history (the `my:history` query operator) |
...
An invalid special value returns a `422` error that identifies the invalid operator.
