...
In order to filter search results to show documents that match certain fields via the [/search REST API](/api/client-api/search/overview), you need to construct a list of `facetFilter` objects and pass it into the [`requestOptions`](/api/client-api/search/search#request) object.
