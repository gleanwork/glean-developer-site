...
You can receive expanded content by setting 2 fields in the request body: [`maxSnippetSize`](/api/client-api/search/search#request) and [`returnLlmContentOverSnippets`](/api/client-api/search/search#request).

## Request Parameters

1. [`maxSnippetSize`](/api/client-api/search/search#request) - Gives a hint to the server on how many characters of expanded content should be returned per result. The server may return less or more. This field must be set if [`returnLlmContentOverSnippets`](/api/client-api/search/search#request) is true, and it must be an integer value greater than 0 and less than or equal to 10000.

2. [`returnLlmContentOverSnippets`](/api/client-api/search/search#request) - If true, the server will return expanded content. If not, the server will return SERP results as expected. The current retrieval process and formatting of content are what we use internally as LLM content, which we are still modifying for internal usage.

## Sample Request

Sample request body (contains all required fields at minimum to get expanded content):

```json snippet=llm-content/snippet-01.json
{
  "query": "mentions", 
  "pageSize": 10, 
  "maxSnippetSize": 4000,
...
```json snippet=llm-content/snippet-02.json
{
  "results": [
    {
      ... more fields...
...
```json snippet=llm-content/snippet-03.json
{
  "results": [
    {
      ... more fields...
