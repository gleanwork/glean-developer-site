...
<CardGroup cols={2}>
  <Card title="Agent Configuration" icon="Cpu">
    Your `/chat` request must set `agentConfig.agent` to `"FAST"` or `"ADVANCED"`.
...
```json snippet=deep-linked-citations/snippet-01.json
{
  "messages": [
    {
      "author": "GLEAN_AI",
...
```json snippet=deep-linked-citations/snippet-02.json
{
  "messages": [
    {
      // ...
...
```json snippet=deep-linked-citations/snippet-03.json
...
```tsx snippet=deep-linked-citations/snippet-04.tsx
...
<Steps titleSize="h3">
  ...
  <Step title="Handle New Fields Defensively">
  ...
  <Step title="Add Contextual Preview (Recommended)">
