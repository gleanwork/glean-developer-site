...
For a per-user chatbot, OAuth is preferred. The Python client's `api_token` field accepts an OAuth access token through the same credential shape shown in this example; use a Glean-issued token only for an approved exception. See [Client API authentication](/api-info/client/authentication/overview) before deploying.

## Installation

```bash snippet=chatbot-example/snippet-01.sh
pip install glean-api-client
```

## Basic Chat Example

```python snippet=chatbot-example/snippet-02.py
import os
...
```bash snippet=chatbot-example/snippet-03.sh
...
```python snippet=chatbot-example/snippet-06.py
import os
from glean.api_client import Glean, models
...
```python snippet=chatbot-example/snippet-07.py
import os
from glean.api_client import Glean, models
...
```python snippet=chatbot-example/snippet-08.py
import logging
from glean.api_client import Glean, models
...
```python snippet=chatbot-example/snippet-09.py
