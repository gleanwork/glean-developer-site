...
Reading presets and triggers requires the `triggers:read` scope; creating, updating and deleting a trigger requires `triggers:write`. A token without them is rejected with `403`, regardless of the experimental header.
