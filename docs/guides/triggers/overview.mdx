...
Reading presets and triggers requires the `TRIGGERS_READ` scope; creating, updating and deleting a trigger requires `TRIGGERS_WRITE`. A token without them is rejected with `403`, regardless of the experimental header.
