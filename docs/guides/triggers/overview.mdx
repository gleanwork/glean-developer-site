...
1. **Pick a preset.** `GET /api/trigger-presets` returns paginated preset summaries in `results`; filter with the `datasource` query parameter, or read the `datasource` field on each result. Preset ids are per-deployment — discover them rather than hardcoding.
2. **Get its inputs.** `GET /api/trigger-presets/{preset_id}` returns the full preset, including the `inputs` array that declares the inputs it accepts and which are required. A preset that takes none returns an empty array.
