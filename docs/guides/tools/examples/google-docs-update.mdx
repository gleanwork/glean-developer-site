...
<Steps>
  <Step title="Basic Configuration">
    <p>Navigate to <a href="https://app.glean.com/admin/platform/tools">Admin console > Platform > Tools</a> and create a new tool from scratch.</p>
    ...
    <ul>
      ...
      <li>Description: Explain the tool's functionality</li>
      <li>Unique identifier: Set a unique ID (e.g., "googleDocsUpdate")</li>
      <li>Tool type: Select "Write" since this will modify documents</li>
    </ul>
    ...
    <p>In the trigger condition, provide comprehensive guidance for when Glean Assistant should use this tool. Include:</p>
    <ul>
      <li>Specific use cases that should trigger the tool</li>
      <li>Examples of appropriate user queries</li>
      <li>Cases where the tool should not be used</li>
    </ul>
  </Step>
...
```yaml snippet=google-docs-update/snippet-01.yaml
openapi: 3.0.0
servers:
...
```
  ...
  <Step title="Authentication Setup">
    ...
    ```
    Client URL: https://accounts.google.com/o/oauth2/v2/auth?access_type=offline&prompt=consent
  ...
  <Step title="Testing and Deployment">
    <p>After configuration, thoroughly test the tool before deployment.</p>
    ...
    4. Verify document changes:
       ...
       Before changes:
       ![State of Doc before Tool Execution](./images/google-docs-update/before_doc.png)

       After changes:
       ![State of Doc after Tool Execution](./images/google-docs-update/after_doc.png)

    <h4>Deployment Configuration</h4>
    <p>Deploy the tool through the deploy tab:</p>
    
    ![Deploy Tool View](./images/google-docs-update/deploy_window.png)

    <p>You can:</p>
    <ul>
      <li>Enable the tool for all teammates</li>
