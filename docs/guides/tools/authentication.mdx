...
![Authentication section in tools configuration](./images/actions_auth_section.png)

<CardGroup cols={3}>
  <Card title="OAuth Admin" icon="UserCheck">
    Requires one-time authorization by the tool developer or app admin during
...
```python snippet=authentication/snippet-01.py
import jwt
import json
import requests
...
```java snippet=authentication/snippet-02.java
