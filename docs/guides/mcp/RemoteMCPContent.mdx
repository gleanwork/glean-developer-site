...
<Admonition type="info" title="Public Beta">
  <p>
    The Remote MCP Server is available to all Glean customers during the public
    beta period.
  </p>
  <p>
    <strong>General Availability:</strong> Targeting mid-late October 2025
  </p>
</Admonition>

## Install Now

Get started in seconds with our quick installer. <a href="#mcp-configurator" onClick={(e) => {
e.preventDefault();
const element = document.getElementById('mcp-configurator');
if (element) {
...
}} style={{ cursor: 'pointer', color: 'var(--ifm-color-primary)' }}>Click here to configure your setup</a> - select your host application and enter your Glean instance details below.

<MCPConfigurator />

<Admonition type="tip">
  <p>
    For ChatGPT Connectors and Claude for Teams/Enterprise, administrator setup
    is required. The configurator will provide instructions when you select
    these hosts.
  </p>
</Admonition>
...
To configure Glean's MCP servers, follow [these setup instructions](https://docs.glean.com/administration/platform/mcp/connecting-to-glean-mcp-server).
