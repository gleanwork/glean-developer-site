...
The Glean OAuth Authorization Server and Remote MCP Server are enabled by default. If the [MCP Configurator](https://app.glean.com/settings/install?mcpConfigure=true) is missing, or it does not show your host, ask a Glean administrator to check tenant policy, the DCR allowlist, and any prior custom MCP or OAuth configuration.
