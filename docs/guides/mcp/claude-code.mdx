...
>
  <PluginPage
    ...
    steps={[
      ...
      {
        type: 'list',
        label: 'Set up the Glean MCP connection',
        description:
        ...
        items: [
          <>
            In Claude Code, prompt: <code>Set up Glean for me</code>
          </>,
    ...
    afterSteps={
      <>
        ...
        <p>
          ...
          <code>env</code> key of a Claude Code <code>settings.json</code>{' '}
          file. Settings files are strict JSON, so merge this key into your
          existing settings rather than pasting comments:
        </p>
  ...
  "env": {
    "USE_CLAUDE_PROJECT_DIR": "1"
        ...
        <ul>
          <li>
            <strong>Every project on this machine</strong> — add it to{' '}
          ...
          <li>
            <strong>Your whole organization</strong> — ask your Claude Code
            admin to set it in managed settings
            (<code>managed-settings.json</code>), which can&rsquo;t be
            overridden locally.
          </li>
        </ul>
        <p>
