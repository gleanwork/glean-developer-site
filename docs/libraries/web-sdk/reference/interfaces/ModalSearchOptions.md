...
Listener invoked when the frame is detached and focus is reverted to the native search box.
...
### showNativeSearchToggle?

```ts
optional showNativeSearchToggle?: boolean;
```

Specifies whether the user should see an option allowing them
to revert back to the native search experience.
