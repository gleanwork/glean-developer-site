...
<Step title="Navigate to Front-end Settings">
  In the **Edit Site** view, select the **Front-End** tab, expand **Advanced**,
  and open **Custom Scripts and Styles**.
</Step>

<Step title="Add Custom Head Elements Container">
  Click **Add Custom Head Elements**, then set **Internal Name** to `Glean`.
</Step>
  ...
  <Step title="Configure Web SDK Script">
    ...
    | **Type** | `Link` | ✅ |
    | **Script URL** | `https://{GLEAN_APP_DOMAIN}/embedded-search-latest.min.js` | ✅ |

    Replace `{GLEAN_APP_DOMAIN}` with your company's Glean app domain, such as `app.glean.com`.
    | **Defer** | `true` | ✅ |
  ...
  <Step title="Add Integration Script">
    ...
    ```javascript snippet=brightspot/snippet-01.js
