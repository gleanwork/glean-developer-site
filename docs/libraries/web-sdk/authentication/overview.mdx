...
import AuthFlow from '@site/src/components/WebSdk/AuthFlow';
...
**Configuration:**
```typescript snippet=overview/snippet-01.ts
{
...
**Configuration:**
```typescript snippet=overview/snippet-04.ts
{
  authMethod: "token",
...
| Do third-party cookies get blocked? | Yes | [Token-Based Authentication](./server-to-server) (recommended) |
...
```bash snippet=overview/snippet-20.sh
npm install @gleanwork/web-sdk
```

Then import in your application:

```typescript snippet=overview/snippet-21.ts
...
```html snippet=overview/snippet-22.html
<script
  defer
...
```javascript snippet=overview/snippet-23.js
...
```javascript snippet=overview/snippet-24.js
GleanWebSDK.renderSearchBox(document.getElementById('search-container'), {
  authMethod: 'token',
