...
{/* Required for docusaurus styles to apply correctly 😬 */}
<div class="markdown">
<h3>Prerequisites</h3>
...
<Steps>
  <Step title="Create Server Endpoint">
    ...
    **Node.js/Express Example:**
    ...
    ```javascript snippet=server-to-server/snippet-01.js
  ...
  <Step title="Install Web SDK">
    ...
    ```html snippet=server-to-server/snippet-02.html
    <script
      defer
    ...
    ```bash snippet=server-to-server/snippet-03.sh
    npm install @gleanwork/web-sdk
    ```

    Import in your application:

    ```typescript snippet=server-to-server/snippet-04.ts
  ...
  <Step title="Implement Client-Side Authentication">
    ...
    ```javascript snippet=server-to-server/snippet-05.js
...
<Steps>
  <Step title="Create Server Endpoint">
    ...
    **Node.js/Express Example:**
    ...
    ```javascript snippet=server-to-server/snippet-06.js
    const express = require("express");
    const axios = require("axios");
    const app = express();
  ...
  <Step title="Install Web SDK">
    ...
    ```html snippet=server-to-server/snippet-07.html
    <script
      defer
      src="https://{GLEAN_APP_DOMAIN}/embedded-search-latest.min.js"
    ></script>
    ...
    ```bash snippet=server-to-server/snippet-08.sh
    npm install @gleanwork/web-sdk
    ```

    Import in your application:

    ```typescript snippet=server-to-server/snippet-09.ts
    import GleanWebSDK from '@gleanwork/web-sdk';
    ```
  ...
  <Step title="Implement Client-Side Authentication">
    ...
    ```javascript snippet=server-to-server/snippet-10.js
    // Fetch initial token from your server
    async function getGleanToken() {
      const response = await fetch('/api/get-glean-token', {
        method: 'POST',
...
<Steps>
  <Step title="Import the Guest Auth Provider">
    ```typescript snippet=server-to-server/snippet-11.ts
    import { createGuestAuthProvider } from '@gleanwork/web-sdk';
    ```
  </Step>

  <Step title="Create the Provider">
    ```typescript snippet=server-to-server/snippet-12.ts
  ...
  <Step title="Use with Web SDK">
    ...
    ```typescript snippet=server-to-server/snippet-13.ts
    // Get the auth token
    ...
    ```tsx snippet=server-to-server/snippet-14.tsx
  ...
  <Step title="Apply to Glean Components">
    ```tsx snippet=server-to-server/snippet-15.tsx
...
- [GuestAuthProvider API](https://app.glean.com/meta/browser_api/interfaces/GuestAuthProvider.html)
