...
Unlike [Default SSO Authentication](./default-sso) which relies on users' existing Glean cookies, server-to-server authentication allows your backend to obtain short-lived authentication tokens on behalf of users. This eliminates the need for users to sign in to Glean directly and enables use cases where users may not even have Glean accounts.
...
#### 1. Import the Guest Auth Provider
