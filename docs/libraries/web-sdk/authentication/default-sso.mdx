...
:::warning
SSO authentication relies on browser cookies. If third-party cookies are blocked or restricted, you must implement [Server-to-Server Authentication](./server-to-server) instead.

Modern browsers (Safari, Firefox, Brave) block third-party cookies by default. For reliable cross-browser support, consider using token-based authentication.
