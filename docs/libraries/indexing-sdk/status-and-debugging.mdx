...
description: Answer "why isn't my document in Glean search?" with StatusClient and glean-index-status
---
...
<Stages stages={[
  ...
  { title: <>Has it finished indexing?</>, body: <>Run <code>glean-index-status --poll</code>. If it&rsquo;s still pending, wait &mdash; large uploads take longer.</> },
  { title: <>Is the datasource healthy?</>, body: <><code>StatusClient.get_datasource_status()</code> shows document counts and datasource state. A count far below what you uploaded points at rejected documents rather than slow indexing.</> },
  { title: <>Can the user see it?</>, body: <>If the document is indexed but one person can&rsquo;t find it, it&rsquo;s a permissions problem. Check the ACL and that the identities it references were pushed.</> },
]} />
...
`glean-index-status` ships with the SDK:
