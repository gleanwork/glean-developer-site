...
<CardGroup cols={1}>
  <Card title="Glean-Issued Tokens (Only Option)" icon="Key">
    **Manually created through admin console** - **Authentication**: Glean
    tokens only (no OAuth support) - **Permissions**: Global by default (no
    user-scoping) - **Scopes**: Full indexing API access (no scope restrictions)
...
<Step title="Add New Token">
  ...
  />
  - **Token Name**: Descriptive name for tracking - **Global Permissions**: Full
  indexing API access - **App Permissions**: Limit to specific datasources
  (optional) - **Expires On**: Optional expiration date - **IP Restrictions**:
  Optional IP range limitations - **Rotation Settings**: Optional automated
  rotation
</Step>
...
<Steps>
  <Step title="Verify token validity">
    - Check that token hasn't expired - Ensure token was created for Indexing
    API (not Client API) - Test with simple endpoint like document count
  </Step>

  <Step title="Check IP restrictions">
    - Verify your IP is in allowed ranges (if IP restrictions enabled) - Test
    from different IP if needed - Check CIDR format in token configuration
  </Step>

  <Step title="Validate request format">
    - Ensure Content-Type header is set correctly - Verify JSON payload is valid
    - Check required fields are present
  </Step>

  <Step title="Test with minimal request">
    - Start with document count or datasource status endpoints - Use minimal
    document for indexing tests - Gradually add complexity once basic auth works
  </Step>
</Steps>
...
<CardGroup cols={3}>
  ...
  <Card
    ...
    href="/api-info/indexing/getting-started/index-documents"
    icon="FileText"
  >
    Learn document indexing best practices
  </Card>
  <Card
    title="Indexing API Reference"
    href="/api/indexing-api/documents-overview"
    icon="Book"
  >
