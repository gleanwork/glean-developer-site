...
<Card title="Custom Datasources" icon="Database">
  Create and manage custom datasources to organize and categorize your indexed
  content
</Card>

<Card title="Structured Entities" icon="Package">
  Index structured key-value content for applications requiring schema-based
  data representation
</Card>
...
<Step title="Index Content">
  Push documents, structured data, and metadata to your datasource using the
  indexing endpoints. Content can be indexed individually or in bulk.
</Step>

<Step title="Manage Permissions">
  Configure access controls to ensure users only see content they're authorized
  to access, respecting your organization's security policies.
</Step>
