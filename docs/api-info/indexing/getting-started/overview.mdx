---
title: 'Indexing API Getting Started'
icon: 'send-backward'
---
...
<Tabs>
  ...
  ```bash snippet=overview/snippet-19.sh
...
- **Super Admins**: Can create indexing tokens, including tokens with `global` scope.
- **Admins**: Can create indexing tokens for any scope except `global`.
- **Setup Admins**: Can create Indexing API tokens (this is the only token type Setup Admins can create).
- **API Token Creators**: Can create indexing tokens. All indexing tokens that API Token Creators create are self-scoped only.
