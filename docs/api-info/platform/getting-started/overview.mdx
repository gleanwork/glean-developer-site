...
The Platform API is Glean's recommended API for new application integrations when the capability and stability level fit the use case. It provides modern endpoints for search, agents, skills, and chat under the `/api` path. Search, Agents, Chat, and Skills are currently experimental.
...
The example below uses the experimental Search API. Review the endpoint's stability notice before using it in production.
...
<CardGroup cols={3}>
  ...
  <Card title="Run agents (Experimental)" icon="agent" iconSet="glean" href="/api/platform-api/agents-overview">
    Discover agents and create agent runs using the experimental Agents API.
  </Card>
</CardGroup>
