...
| `webhook-timestamp` | Unix seconds. Reject deliveries outside a tolerance window; 300 seconds is the Standard Webhooks recommendation. |
...
- Deliveries are filtered per event against the subscribing user's document permissions, so a trigger never surfaces a document its owner could not already open.
