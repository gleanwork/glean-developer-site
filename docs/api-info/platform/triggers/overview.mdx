...
Both search endpoints are read-only: nothing is created and no webhook is delivered. `POST /trigger-presets/{preset_id}/events/search` previews an unsaved preset plus inputs; `POST /triggers/{trigger_id}/events/search` replays what an existing trigger has been matching.

Three limits to plan for:

- They cover the **last seven days** only.
