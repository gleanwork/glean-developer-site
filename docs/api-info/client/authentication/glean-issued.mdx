...
<Card title="Glean Token Benefits" icon="Key">
  ...
  **User-scoped or global** - Choose appropriate permission level - **No
  external dependencies** - Works without identity provider setup - **Flexible
  expiration** - Set custom expiry dates
</Card>
...
<Step title="Add New Token">
  Click **Add New Token** and configure: - **Description**: Helpful name for
  tracking - **Permissions**: USER (recommended) or GLOBAL - **Scopes**: Select
  required API access scopes - **Expires**: Optional expiration date - **User
  Email**: Required for USER tokens
</Step>
...
<Steps>
  ...
  <Step title="Confirm headers">
    Ensure you have the correct headers for your token type: - User token:
    `Authorization` only - Global token: `Authorization` + `X-Glean-ActAs`
  </Step>
