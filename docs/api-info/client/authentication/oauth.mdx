...
<CardGroup cols={2}>
  <Card title="Glean OAuth Authorization Server" icon="Shield">
    **Glean issues the tokens (OAuth 2.1)**
    ...
    - Static clients for application integrations; Dynamic Client Registration (DCR) only when enabled and supported
...
<Steps>
  <Step title="Confirm the authorization-server prerequisites">
    The Glean OAuth Authorization Server must be enabled for your tenant. Ask a Glean administrator to verify that it is available. Glean continues to use your existing SSO for user authentication; it does not replace your IdP.
  </Step>

  <Step title="Register the right kind of client">
    Register a [static OAuth client](https://docs.glean.com/administration/oauth/static-client-registration) for an application integration. [Dynamic Client Registration (DCR)](https://docs.glean.com/administration/oauth/dynamic-client-registration) is available only when your administrator enables it and the application supports it. Administrators can restrict DCR by application or redirect URI, or disable it entirely.
  </Step>

  <Step title="Obtain a token">
    Follow the [static-client token example](https://docs.glean.com/administration/oauth/static-client-token-example) and use the Authorization Code flow with **PKCE**. Discover endpoints from the server metadata document and exchange the authorization code for an access token.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="Glean OAuth Authorization Server" href="https://docs.glean.com/administration/oauth/authorization-server" icon="Shield">
    Admin guide to enabling the server and registering static OAuth clients
  </Card>
