...
<CardGroup cols={2}>
  <Card title="OAuth (Recommended for per-user integrations)" icon="Shield">
    **Glean Authorization Server or your IdP**
    ...
    - Use static clients for application integrations; DCR is conditional
...
### Choose OAuth if you:
- Are building a per-user Client API integration and want standards-based, user-authorized access without manually managing Glean API tokens
- Can use a static client with the Glean OAuth Authorization Server, or already use supported OAuth tokens from your enterprise identity provider
- Want the token issuer to manage expiration and refresh
- Are building a user-facing integration

### Choose Glean Tokens if you:
- Need an administrator-managed token lifecycle or a supported server-to-server exception
- Cannot use OAuth because neither the Glean OAuth Authorization Server is enabled nor an external-IdP OAuth path is configured
...
```bash snippet=overview/snippet-02.sh
...
```bash snippet=overview/snippet-03.sh
...
```bash snippet=overview/snippet-04.sh
...
Use the [Search API Explorer](/api/client-api/search/search) to test a Client API request against your own tenant. API Explorer is an interactive tenant-testing tool, not production sample code; implement the OAuth flow and token handling in your application before deploying it.
...
```bash snippet=overview/snippet-05.sh
...
```bash snippet=overview/snippet-06.sh
curl -X POST https://<instance>-be.glean.com/rest/api/v1/search \
...
```bash snippet=overview/snippet-07.sh
curl -X POST https://<instance>-be.glean.com/rest/api/v1/search \
...
```json snippet=overview/snippet-08.json
{
  "results": [...],
  "trackingToken": "...",
  "requestId": "..."
...
<CardGroup cols={2}>
  ...
  <Card title="Datasource OAuth Redirects" href="./datasource-oauth" icon="ExternalLink">
