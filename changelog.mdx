...
We fixed a bug that caused [Run Agent Wait for Output](/client/api/agents/create-run-wait-for-output) (`/agents/run/wait`) to return empty responses when used with
the API Client libraries. We also fixed a bug that produced non-SSE compliant output when using the [Run Agent Stream
Output](/client/api/agents/create-run-stream-output) (`/agents/run/stream`) endpoint.
</Update>
