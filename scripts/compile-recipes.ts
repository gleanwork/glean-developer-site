...
export function compileRecipeFacets(records: RecipeRecord[]): {
  capabilities: RecipesData['capabilities'];
  surfaces: RecipesData['surfaces'];
} {
  const taxonomyOrder = (order: readonly string[]) => (a: string, b: string) =>
    order.indexOf(a) - order.indexOf(b);

  return {
    capabilities: [
      ...new Set(records.flatMap((record) => record.capabilities)),
    ].sort(taxonomyOrder(RECIPE_CAPABILITIES)),
    surfaces: [...new Set(records.flatMap((record) => record.surfaces))].sort(
      taxonomyOrder(RECIPE_SURFACES),
    ),
  };
}
...
function main(): void {
  ...
  const facets = compileRecipeFacets(records);
