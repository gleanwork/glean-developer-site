...
class DeterministicCircularResolver {
    ...
    detectCircularReferences(schemas) {
        /**Detect all circular references - IMPROVED VERSION matching Python logic*/
        this.buildSchemaGraph(schemas);
        this.printSchemaGraph();
        
        const visitedGlobal = new Set();
        const circularSchemas = new Set();
        const circularPaths = {}; // schema_name -> list of cycle paths
        const allPaths = {}; // schema_name -> set of paths
        const uniqueCycles = new Set(); // set of normalized cycles
        
        const normalizeCycle = (cyclePath) => {
            ...
            const cycleWithoutEnd = cyclePath.slice(0, -1);
            
            // Find the lexicographically smallest starting point
            const minElement = cycleWithoutEnd.reduce((min, current) => current < min ? current : min);
            const minIdx = cycleWithoutEnd.indexOf(minElement);
            
            // Rotate the cycle to start from the smallest element
            const normalized = cycleWithoutEnd.slice(minIdx).concat(cycleWithoutEnd.slice(0, minIdx));
        ...
        const dfs = (node, path, visited, origin) => {
            /**DFS that properly tracks all paths and cycles*/
            // Track all traversed paths from origin
            if (!allPaths[origin]) {
                allPaths[origin] = new Set();
            }
            allPaths[origin].add([...path, node].join(' -> '));
            
            if (visited.has(node)) {
                // Found a cycle
                const cycleStart = path.indexOf(node);
                const cycle = new Set(path.slice(cycleStart));
                for (const schema of cycle) {
                ...
                // Track cycle paths for each schema in the cycle
                for (const schema of cycle) {
                    if (!circularPaths[schema]) {
                        circularPaths[schema] = [];
                    }
                    circularPaths[schema].push(cyclePath);
                }
                return;
            }
        ...
        for (const schemaName of Object.keys(schemas).sort()) {
            if (!visitedGlobal.has(schemaName)) {
                dfs(schemaName, [], new Set(), schemaName);
        ...
        for (const [schemaName, schema] of Object.entries(schemas)) {
            ...
            if (refs.has(schemaName)) {
                ...
                const cyclePath = [schemaName, schemaName];
                const normalizedCycle = normalizeCycle(cyclePath);
                uniqueCycles.add(normalizedCycle);
                
                if (!circularPaths[schemaName]) {
                    circularPaths[schemaName] = [];
                }
                circularPaths[schemaName].push(cyclePath);
                
                if (!allPaths[schemaName]) {
                    allPaths[schemaName] = new Set();
                }
                allPaths[schemaName].add([schemaName, schemaName].join(' -> '));
            }
        }
        
        // Print detailed results like the Python script
        console.log(`\nUnique Circular Paths (${uniqueCycles.size} total):`);
        const sortedCycles = Array.from(uniqueCycles).sort();
        ...
        for (const schemaName of Object.keys(schemas).sort()) {
            if (circularSchemas.has(schemaName)) {
                console.log(`- ${schemaName} (circular reference)`);
            } else {
                const pathCount = allPaths[schemaName] ? allPaths[schemaName].size : 0;
                console.log(`- ${schemaName}: ${pathCount} unique paths traversed (no circular reference)`);
            }
        }
        
        console.log(`\nTotal schemas with circular references detected: ${circularSchemas.size}`);
        console.log(`Total unique circular paths: ${uniqueCycles.size}`);
    ...
    resolveSchemaObjectSelectively(obj, allSchemas, visited, depth = 0, inArray = false) {
        ...
        if (obj.type === 'array' && 'items' in obj) {
            resolved['type'] = 'array';
            const resolvedItems = this.resolveSchemaObjectSelectively(obj['items'], allSchemas, visited, depth, true);
        ...
        if ('properties' in obj) {
            ...
            for (const propName of Object.keys(obj['properties']).sort()) {
                ...
                try {
                    // Don't increment depth for properties, only for actual schema references
                    resolved['properties'][propName] = this.resolveSchemaObjectSelectively(
                        propDef, allSchemas, visited, depth, false
                    );
                } catch (e) {
        ...
        if ('allOf' in obj) {
            ...
            for (let i = 0; i < obj['allOf'].length; i++) {
                ...
                try {
                    const resolvedSub = this.resolveSchemaObjectSelectively(subSchema, allSchemas, visited, depth, false);
        ...
        if ('oneOf' in obj) {
            ...
            for (let i = 0; i < obj['oneOf'].length; i++) {
                ...
                try {
                    resolved['oneOf'].push(this.resolveSchemaObjectSelectively(subSchema, allSchemas, visited, depth, false));
                } catch (e) {
        ...
        if ('anyOf' in obj) {
            ...
            for (let i = 0; i < obj['anyOf'].length; i++) {
                ...
                try {
                    resolved['anyOf'].push(this.resolveSchemaObjectSelectively(subSchema, allSchemas, visited, depth, false));
                } catch (e) {
