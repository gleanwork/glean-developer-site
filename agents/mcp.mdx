  ...
  <Tab title="Configure using the CLI">
    <Steps>
      ...
      <Step title="Configure Cursor">
        ...
        ```bash
        npx @gleanwork/mcp-server configure --client cursor --instance <your-glean-subdomain> --token <your-glean-api-token>
        ```
  ...
  <Tab title="Configure using the CLI">
    <Steps>
      ...
      <Step title="Configure Windsurf">
        ...
        Using explicit `domain` and `token` flags:
        ```bash
        npx @gleanwork/mcp-server configure --client windsurf --instance <your-glean-subdomain> --token <your-glean-api-token>
        ```
...
<Tabs>
  <Tab title="Configure using the CLI">
    <Steps>
      ...
      <Step title="Configure VS Code">
        Run the following command to configure VS Code to use Glean's MCP server. This will add a new MCP server to VS Code's global settings.

        Using explicit `domain` and `token` flags:
        ```bash
        npx @gleanwork/mcp-server configure --client vscode --domain <your-glean-subdomain> --token <your-glean-api-token>
        ```
  ...
  <Tab title="Configure manually">
    <Steps>
      ...
      <Step title="Choose Configuration Mode">
        VS Code supports two modes for configuring MCP servers:
        
        - **Workspace Configuration**: Stores configuration in `.vscode/mcp.json` and shares it with your team
        - **Global Configuration**: Stores configuration in your personal VS Code settings for all workspaces

        Choose the approach that best fits your needs below.
      </Step>
      ...
      <Step title="Configure VS Code">
        **Workspace Configuration (Recommended for teams)**
        ...
        ```json
        {
          ...
          "servers": {
            "glean": {
              ...
              "args": ["-y", "@gleanwork/mcp-server"],
              "env": {
                "GLEAN_SUBDOMAIN": "${input:glean-subdomain}",
        ...
        5. VS Code will prompt you for the subdomain and API token when the server first starts
  ...
  <Tab title="Configure using the CLI">
    <Steps>
      ...
      <Step title="Configure Claude Desktop">
        ...
        Using explicit `domain` and `token` flags:
        ```bash
        npx @gleanwork/mcp-server configure --client claude --instance <your-glean-subdomain> --token <your-glean-api-token>
        ```
